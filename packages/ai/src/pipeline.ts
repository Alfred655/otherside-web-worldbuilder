...
export class GenerationPipeline {
  ...
  async generate(
    ...
    // ── Step 1: Generator (merged Designer + Builder) ──────────────────
    onProgress?.("Designing and building your world...");
    console.log("[Pipeline:Generator] Generating GameSpec JSON...");
    const rawSpec = await this.runGenerator(prompt, template, onProgress);
    console.log(
      `[Pipeline:Generator] Generated "${rawSpec.name}" with ${rawSpec.entities.length} entities`,
    );
  ...
  // ── Generator (merged Designer + Builder) ─────────────────────────────
  ...
  private async runGenerator(
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      ...
      let userContent = `Create a complete playable 3D game for: "${prompt}"`;
      if (template) {
      ...
      if (response.stop_reason === "max_tokens") {
        lastError =
          "Output was truncated (exceeded max_tokens). Reduce to 8 entities max. Use primitive meshes instead of compound for collectibles and props.";
        lastFailedJSON = null;
        console.warn(
          `[Pipeline:Generator] Attempt ${attempt + 1}/${MAX_RETRIES} truncated (stop_reason=max_tokens)`,
        );
        continue;
      }
      ...
      try {
        ...
        lastFailedJSON = json;
        console.warn(
          `[Pipeline:Generator] Attempt ${attempt + 1}/${MAX_RETRIES} validation failed:\n`,
        ...
        lastFailedJSON = json;
        console.warn(
          `[Pipeline:Generator] Attempt ${attempt + 1}/${MAX_RETRIES} failed:`,
          msg.slice(0, 200),
        );
      }
    }
  ...
  private async runValidator(
    ...
    try {
      ...
      if (response.stop_reason === "max_tokens") {
        console.warn(
          "[Pipeline:Validator] LLM fix truncated, using programmatic fix",
        );
        return fixed;
      }
